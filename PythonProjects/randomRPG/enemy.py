...
class Enemy(Character):
    ...
    def levelUp(self, player):
        ...
        if self.isBoss is True:
            self.isBoss = False
        if player.level % 5 == 0:
            self.generateBoss(player)
        else:
            while self.level < player.level:
                self.level = self.level + 1
                self.luck = (self.luck + self.randomize(1, self.level)) + self.randomize(round(self.scale / 3),
                                                                                         self.scale)
                ...
                self.maxMP = (self.maxMP + self.randomize(0, 3)) + self.randomize(round(self.scale / 16),
                                                                                  round(self.scale / 8))
    ...
    def generateBoss(self, player):
        self.reset()
        self.isBoss = True
        while self.level < player.level + 5:
            self.level = self.level + 1
            self.luck = (self.luck + self.randomize(1, round(self.level / 2.3333))) + self.randomize(
                round(self.scale / 3), self.scale)
            self.attack = (self.attack + self.randomize(1, round(self.level / 3))) + self.randomize(
                round(self.scale / 8), round(self.scale / 2))
            self.defence = (self.defence + self.randomize(1, round(self.level / 1.5))) + self.randomize(
                round(self.scale / 4),
                self.scale)
            self.speed = (
                    self.speed + self.randomize(1, round(self.level / 1.5)) + self.randomize(round(self.scale / 3),
                                                                                             self.scale))
            self.maxHP = (self.maxHP + self.randomize(2, 10)) + self.randomize(round(self.scale / 2), self.scale)
            self.maxMP = (self.maxMP + self.randomize(1, 2)) + self.randomize(round(self.scale / 16),
                                                                              round(self.scale / 8))
        self.currentHP = self.maxHP
        self.currentMP = self.maxMP
        self.name = self.generateName()
    ...
    def generateName(self):
        names = ["Kevin", "Cryptpaw", "Shade", "Hauntsoul", "Rotwing", "Crabman", "Moldtaur", "Handvine", "Spiteling",
                 "Lich", "Firepaw", "Dogbot", "Wraith", "Blaze", "Rotclaw", "Umbrah", "Armorpaw", "Goblin", "Catten",
                 "Obu", "Red Slime", "Gauss", "Dozar", "Iago", "Ink Eel", "Prowler", "Slurpem", "Python", "Boombat",
                 "Darkpaw", "Eyecell"]
